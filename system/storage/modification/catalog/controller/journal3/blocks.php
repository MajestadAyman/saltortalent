      ...
      if ($this->config->get('mlseo_enabled')) {
        ...
        if ($this->config->get('mlseo_autolink')) {
          ...
          foreach ($autolinks as $autolink) {
            static::$PRODUCT_INFO['description'] = preg_replace('/<a\b[^>]*>[^<]*<\/a>(*SKIP)(*FAIL)|\b('.$autolink['query'].')\b/', '<a href="'.$autolink['redirect'].'">$1</a>', static::$PRODUCT_INFO['description']);
          }
        }
        
        static::$PRODUCT_INFO['description'] = $extraTopDesc . static::$PRODUCT_INFO['description'] . $extraBottomDesc;
      }
