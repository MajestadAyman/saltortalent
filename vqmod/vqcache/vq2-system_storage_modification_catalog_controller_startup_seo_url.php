...
class ControllerStartupSeoUrl extends Controller {
	public function index() {
      ...
      */
      ...
      if ($this->config->get('advanced_sitemap_rewrite')) {
        ...
        if ($uri == $sitemapFolder.'sitemap.xml') {
          ...
          if (preg_match('/^((\w{2}-\w{2})|\w{2})$/', $advStmpVars[1])) {
            ...
            if (!empty($advStmpVars[2]) && empty($advStmpVars[3]) && strpos($advStmpVars[2], '-') !== false) {
              $expl = explode('-', $advStmpVars[2]);
              $this->request->get['lang'] = $expl[0];
              $advStmpVars[3] = $expl[1];
            } else {
      ...
      if (empty($this->request->get['_route_']) && empty($this->request->get['route']) && strpos($_SERVER['REQUEST_URI'], 'index.php/') !== false) {
      ...
      */
      ...
      foreach ($parts as $part) {
        ...
        if ($this->config->get('mlseo_preload')) {
          $query = $this->getSeoUrl($this->db->escape($part));
        } else {
            ...
            if ($this->config->get('mlseo_enabled') && $this->config->get('mlseo_absolute') && $query->num_rows > 1) {
              ...
              if ($this->config->get('mlseo_fpp_brand_parent') && isset($this->request->get['route']) && $this->request->get['route'] == 'product/manufacturer') {
                unset($this->request->get['route']);
					...
					if (empty($this->request->get['route']) && !strpos($query->row['query'], '=') && $url[0] != 'route' && isset ($query->row['query']) && $query->row['query'] && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id') {
						$this->request->get['route'] = $query->row['query'];
					}

      } else if (count($parts) > $partsIteration) {
  ...
  public static $seo_urls;
  
  public function getSeoUrl($key) {
    if (self::$seo_urls == null) {
      ...
      foreach ($query->rows as $row) {
        self::$seo_urls[$row['query']] = $row['keyword'];
      }
    }
    
    $resUrl = false;
    
    if (isset(self::$seo_urls[$key])) {
      $resUrl = new stdClass();
      $resUrl->num_rows = 1;
      $resUrl->row = array('keyword' => self::$seo_urls[$key]);
    }
	...
	public function rewrite($link) {
        ...
        if ($this->config->get('mlseo_enabled') && !($data['route'] == 'product/product' && $key == 'manufacturer_id')) $url .= '/' . $query->row['keyword']; else
                ...
                } elseif (isset($data['route']) && $data['route'] == 'journal3/blog' && !isset($data['journal_blog_tag'])) {
            ...
            unset($data['page']);
          } elseif ($this->config->get('mlseo_enabled') && $this->config->get('mlseo_sort') && $key == 'limit' && $url) {
            ...
            unset($data['limit']);
          } elseif ($this->config->get('mlseo_enabled') && $this->config->get('mlseo_sort') && $key == 'sort' && $url) {
            ...
            if (version_compare(VERSION, '3', '>=')) {
              if ($this->config->get('mlseo_preload')) {
                $query = $this->getSeoUrl($this->db->escape($key . '=' . $value));
              } else {
            ...
            if (isset($query) && isset($query->num_rows) && ${'query'}->num_rows) {
              $url .= '/' . $query->row["keyword"];
        ...
        if ($this->config->get('mlseo_preload')) {
          $query = $this->getSeoUrl("category_id=" . (int)$category);
        } else {
